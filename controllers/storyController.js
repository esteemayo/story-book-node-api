    ...
    if (story.status === 'public') {
        ...
        if (req.user) {
            if (req.user.username === story.author) {
                ...
                return next(new ForbiddenError('You do not have the permission to view this story'));
            }
        } else {
            return next(new UnauthenticatedError('You have to be logged in to perform this action'));
        }
    }
});
